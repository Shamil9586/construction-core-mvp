 ...
 * defaults (60000ms / 300 global / 30 on /auth). A present but unparsable value
 * aborts startup instead of silently becoming NaN — express-rate-limit treats a
 * NaN limit as no limit at all, so failing closed is the only safe reading.
 */
export function positiveIntEnv(name: string, fallback: number) { const raw = process.env[name]?.trim(); if (raw === undefined || raw === '')
    return fallback; if (!/^[1-9][0-9]*$/.test(raw) || !Number.isSafeInteger(Number(raw)))
    throw Error(name + ' must be a positive integer'); return Number(raw); }
