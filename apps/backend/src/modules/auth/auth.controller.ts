...
export class AuthController {
    ...
    b: any,
    @Res()
    res: any) { ensure(r.headers.origin === `https://${process.env.BITRIX_PORTAL}`, 'Invalid portal origin'); const result = await installBitrix(this.bitrixPayload(r, b)); this.launchHtml(res, result, true); }
    ...
    b: any,
    @Res()
    res: any) { ensure(r.headers.origin === `https://${process.env.BITRIX_PORTAL}`, 'Invalid portal origin'); const result = await bitrixLogin(this.bitrixPayload(r, b)); this.launchHtml(res, result, false); }
