# Тестовое развёртывание (self-hosted Docker Compose)

Этот документ описывает self-hosted трек на произвольном Linux-хосте с
Docker Compose. Он не был исполнен в текущей итерации. Фактически
задеплоенная и полностью проверенная test-среда — Render, задокументирована
отдельно в [`deployment-render.md`](deployment-render.md); там же — два
платформенных бага и их фиксы, актуальные live-URL и env-конфигурация.

Docker/Compose конфигурация ниже подготовлена, но не запускалась в данной среде. Не считать её проверенной до выполнения на тестовом хосте.
